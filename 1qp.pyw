...
workbook = openpyxl.load_workbook(r"C:\\Solution\\Temp1.xlsx")
...
workbook.close()
workbook = openpyxl.load_workbook(r"C:\\Solution\\Temp2.xlsx")
sheet = workbook.active
if sheet.cell(sheet.max_row,14).value=="-":
    sheet.delete_rows(sheet.max_row)
for i in range(1,sheet.max_row+1):
    (sheet.cell(i,9).value,sheet.cell(i,6).value)=(sheet.cell(i,6).value,sheet.cell(i,9).value)
    (sheet.cell(i,10).value,sheet.cell(i,7).value)=(sheet.cell(i,7).value,sheet.cell(i,10).value)
    (sheet.cell(i,12).value,sheet.cell(i,11).value)=(sheet.cell(i,11).value,sheet.cell(i,12).value)
    (sheet.cell(i,12).value,sheet.cell(i,13).value)=(sheet.cell(i,13).value,sheet.cell(i,12).value)
...
for i in range(2, nrows + 1):
    ...
    if sheet.cell(i, 2).value is None and sheet.cell(i, 4).value is not None:
        sheet.cell(i, 3).value = sheet.cell(i - 1, 3).value

    if sheet.cell(i, 4).value is not None:
        if sheet.cell(i, 3).value is None:
            sheet.cell(i, 3).value = ""
        if sheet.cell(i, 5).value is None:
            sheet.cell(i, 5).value = ""
        specific_dim.append(str(sheet.cell(i, 3).value) + str(sheet.cell(i, 4).value) + str(sheet.cell(i, 5).value))
    elif sheet.cell(i, 4).value is None:
        specific_dim.append(sheet.cell(i, 4).value)
    else:
        specific_dim.append(str(sheet.cell(i, 4).value))

    instrument.append(sheet.cell(i, 6).value)
    if sheet.cell(i,3).value=="Eq==":
        sheet.cell(i,3).value="'Eq=="
    symbols.append(sheet.cell(i, 3).value)
    drg_zone.append(sheet.cell(i, 8).value)
    drwtolminus.append(str(sheet.cell(i, 9).value))
    drwtolplus.append(str(sheet.cell(i, 10).value))
    tolplus.append(sheet.cell(i, 12).value)
    tolminus.append(sheet.cell(i, 11).value)
    remarks.append(sheet.cell(i, 13).value)
    slno.append(sheet.cell(i, 1).value)
    least_count.append(sheet.cell(i, 7).value)
    infpr.append(sheet.cell(i,14).value)
drwtol = []
for i in range (0,len(drwtolplus)):
    if (drwtolplus[i] is None or drwtolplus[i] == "None") and (drwtolminus[i] is None or drwtolminus[i] == "None"):
        drwtol.append(None)
    else:
        if drwtolplus[i] is None or drwtolplus[i] == "None":
            drwtolplus[i] = "-"
        if drwtolminus[i] is None or drwtolminus[i] == "None":
            drwtolminus[i] = "-"
        
        if drwtolplus[i] == "-" and drwtolminus[i] != "-":
            drwtol.append(str(drwtolplus[i])+"/"+str(drwtolminus[i]))
        if drwtolminus[i] == "-" and drwtolplus[i] != "-":
            drwtol.append(str(drwtolplus[i])+"/"+str(drwtolminus[i]))
        if drwtolplus[i] == "-" and drwtolminus[i] == "-":
            drwtol.append("-")
        if drwtolplus[i] != "-" and drwtolminus[i] != "-":
...
workbook = openpyxl.load_workbook(r"C:\\Solution\\Temp3.xlsx")
sheet = workbook.active
...
worksheet1.cell(6, 3).value = "DRG.No:" + input_1[2] + "        ,Rev:" + input_1[3] + "        ,Part No:" + input_1[4]
...
worksheet1.cell(31, 3).value = input_1[14]
worksheet1.cell(32, 1).value = "PART NAME: " + input_1[1]
worksheet1.cell(32, 8).value = "DRG.No:" + input_1[2] + "        ,Rev:" + input_1[3] + "        ,Part No:" + input_1[4]
...
while (j<max_size):
    ...
    if allow == 0:
        ...
        if i - 1 in skip and i - 1 != 8:
            ...
            worksheet1.cell(i - 2, 8).value = "DRG.No:" + input_1[2] + "        ,Rev:" + input_1[3] + "        ,Part No:" + input_1[4]
...
worksheet1.cell(ab + 1, 3).value = input_1[14]
worksheet1.cell(ab + 2, 1).value = "PART NAME: " + input_1[1]
worksheet1.cell(ab+ 2, 8).value = "DRG.No:" + input_1[2] + "        ,Rev:" + input_1[3] + "        ,Part No:" + input_1[4]
...
workbook1.save(path+"/"+input_1[1]+"-"+input_1[2]+"-"+input_1[3]+".xlsx")    
pyautogui.alert("QP IS READY","QP")
